...
public:
   ringlist(); //конструктор
   ~ringlist(); //деструктор
   ringlist(const ringlist<valtype> &l); //конструктор копирования

   void gotohead(); //переход на голову списка
   void gotonext(); //переход к следующему элементу списка
   bool nowhead() const; //проверка на конец списка

   void cleanlist(); //очистка списка
   void elemdelete(const node<valtype>* n); //удаление элемента по указателю
   ...
   bool operator==(const ringlist<valtype> &l) const; //проверка на равно
   bool operator!=(const ringlist<valtype> &l) const; //проверка на неравно
   ...
   friend ostream & operator<<(ostream &out, const ringlist<valtype> &l) //перегрузка оператора вывода
   {

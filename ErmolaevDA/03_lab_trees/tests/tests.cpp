...
{ ScanTable <int,string> T1; 
  ...
  for (int i = 0; i < 4; i++)
  {
     ...
     T1.InsertRecord((i+1) * 8 + (i * i) % 13, s); }   
  ScanTable SS  A(T1);
  A.Reset();
...
{ ScanTable SS  T1; 
  for (int i = 0; i < 4; i++)
  { T1.InsertRecord((i+1) * 8 + (i * i) % 13, to_string(i+1) + " record"); }    
  ASSERT_NO_THROW(T1.InsertRecord(2, "2 record"));
}
...
{ SortTable SS  T1;
  for (int i = 0; i < 4; i++)
  { T1.InsertRecord((i+1), to_string((i+1) * 8 + (i * i) % 13) + " record"); }  
  T1.Reset();
  ASSERT_NO_THROW(*(T1.GetData()));
}
...
{ SortTable SS  T1; 
  for (int i = 0; i < 4; i++)
  { T1.InsertRecord((i+1), to_string((i+1) * 8 + (i * i) % 13) + " record"); } 
  T1.RemoveRecord("8 record");
  T1.RemoveRecord("17 record");
  T1.RemoveRecord("28 record");
  T1.RemoveRecord("41 record");
  EXPECT_EQ(true, T1.IsEmpty());
}
...
{ SortTable SS  T1; 
  for (int i = 0; i < 4; i++)
  { T1.InsertRecord((i+1), to_string((i+1) * 8 + (i * i) % 13) + " record"); }    
  EXPECT_EQ(3, *(T1.FindRecord("28 record")));
}
TEST(SortTable, can_not_Insert_record_wich_key_isnt_unique2)
{ SortTable SS  T1; 
  for (int i = 0; i < 4; i++)
  { T1.InsertRecord((i+1), to_string((i+1) * 8 + (i * i) % 13) + " record"); }    
  ASSERT_NO_THROW(T1.InsertRecord(2, "28 record"));
}
...
{ HashTable SS  T1;
  ...
  { T1.InsertRecord((i+1) * 8 + (i * i) % 13, to_string(i+1) + " record"); }  
  T1.Reset();
  ASSERT_NO_THROW(*(T1.records[0]->data));
}
TEST(HashTable, GetData_corect3)
{ HashTable SS  T1; 
  for (int i = 0; i < 4; i++)
  { T1.InsertRecord(i, to_string(i+1) + " record"); }  
  T1.Reset();
  T1.SetNext();
  EXPECT_EQ(2, *T1.GetData());
}
...
{ HashTable SS  T1; 
  for (int i = 0; i < 4; i++)
  { T1.InsertRecord((i+1) * 8 + (i * i) % 13, to_string(i+1)); }
  T1.RemoveRecord("1");
  T1.RemoveRecord("2");
  T1.RemoveRecord("3");
  T1.RemoveRecord("4");

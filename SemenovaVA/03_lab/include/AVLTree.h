...
{ switch (N1->GetBalance()) 
  { case 1: N1->SetBalance(0);   // в поддереве был перевес справа
           break;               // устанавливается равновесие
    case 0: N1->SetBalance(-1);  // в поддереве было равновесие
    ...
    case -1:                     // в поддереве был перевес слева - необходима балансировка
           ...
           if (p1->GetBalance() == -1)   // случай 1 - однократный LL поворот
           { //1
             if (p1->Right!= nullptr)
             { (*(p1->Right)).Parent = N1; }
             N1->Left = p1->Right;  
...
{ switch (N1->GetBalance())
  { case -1: N1->SetBalance(0);        // в поддереве был перевес слева
             break;                    // устанавливается равновесие
    case 0:  N1->SetBalance(1);        // в поддереве было равновесие
    ...
    case 1:  // в поддереве был перевес справа - необходима балансировка
             ...
             if (p1->GetBalance() == 1) // случай 1 - однократный RR поворот
             { //1
               if (p1->Left != nullptr)
               { (*p1->Left).Parent = N1; }
               N1->Right = p1->Left; 

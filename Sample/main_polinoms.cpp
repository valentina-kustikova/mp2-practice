	...
	while (t != 0)
	{
		...
		case 7:
		{
			...
			if (d3 == 1) {
				...
				do {
					ita++;
					res += ((*ita).coef) * (pow(x, ((*ita).degx))) * (pow(y, ((*ita).degy))) * (pow(z, ((*ita).degz)));
				} while ((*ita).next != NULL);
			...
			if (d3 == 2) {
				...
				do {
					itb++;
					res += ((*itb).coef) * (pow(x, ((*itb).degx))) * (pow(y, ((*itb).degy))) * (pow(z, ((*itb).degz)));
				} while ((*itb).next != NULL);
				cout << "Результат res=" << res << endl;
				res = 0;
			...
			if (d3 == 3) {
				if (S2 == 0) cout << "SAVE пустой, сначала посчитайте что-нибудь и сохраните!" << endl;
					...
					do {
						its++;
						res += ((*its).coef) * (pow(x, ((*its).degx))) * (pow(y, ((*its).degy))) * (pow(z, ((*its).degz)));
					} while ((*its).next != NULL);
					cout << "Результат res=" << res << endl;
					res = 0;
				}

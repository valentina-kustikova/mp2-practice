...
template <typename T>
TList<T>::TList(TNode<T>* _pFirst) : TList<T>() {
...
template <typename T>
TList<T>::TList(const TList<T>& obj) : TList<T>() {
	if (obj.IsEmpty()) return;

	TNode<T>* tmp = obj.pFirst->pNext;
...
void TList<T>::InsertAfter(const T& data) {
	...
	pCurr->pNext = new TNode<T>(data, tmp);

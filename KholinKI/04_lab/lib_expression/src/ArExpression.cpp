...
map<string, double> ArithmeticExpression::SetOperands_v2
(const vector<string> operands, const map<string, double> your_values) {
	...
	while (it_begin != it_end) {
		...
		tmp.insert({ *it_begin, your_values.at(*it_begin)});
		it_begin++;
	}
	return tmp;
}
...
(const map<string, double>& values) {
	...
	while (it_begin != it_end) {
		...
		case '^':
		{
			right_op = expr_operands.Pop();
			left_op = expr_operands.Pop();
			expr_operands.Push(Quadruple(left_op,right_op));
			break;
		}
		default:

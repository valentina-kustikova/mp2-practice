...
int Postfix::GetPriority(const char op)
{
    ...
    case '/':
        return 3;
    default:
        throw "ќшибка. Ќеправильный символ.";
    }
}
...
Postfix::Postfix(const std::vector<std::string>& postf)
{
    ...
    for (int i = 0; i < postf.size(); i++)
    {
        ...
        if (!isOperator(postf[i]) && operands.find(postf[i]) == operands.end()) {
            ...
            // поместить новый уникальный операнд в map
...
{
    std::cout << "¬ведите значение переменной дл€ \"" << name << "\"" << std::endl;
    double value = INFINITY;
    std::cin >> value;
...
{
    TStack<double> S; // сохраненное значение
    for (int i = 0; i < postfixArray.size(); i++)
    {
        if (!isOperator(postfixArray[i])) // если операнд
        {
...
// ѕолучить строковое им€ операнда в исходных строках
...
Postfix Postfix::ConvertToPol(const std::string& s)
{
    ...
    // мы получили постфиксную запись в стеке 1, теперь преобразуем ее в строку
...
void Postfix::setValuesFromVector(const std::vector<double>& values) {
    ...
    {
        std::cout << " оличество предоставленных значений не равно количеству не заданных значений\n";
        return;
    }

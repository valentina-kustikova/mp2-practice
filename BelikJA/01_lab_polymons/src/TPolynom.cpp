...
TPolynom::TPolynom(const TList<unsigned int, double>* tm)
{
    ...
    pol = new TList<unsigned int, double>(tmp->pFirst);
    CheckLim(tmp);
    tmp->Reset();
    tmp->Next();
    int conv = tmp->pFirst->key;
    while (!tmp->IsEnded())
    {
        if (tmp->pCurr->key < conv)
        {
            pol->InsertBefore(*tmp->pCurr, conv);
        }
        else
        {
            pol->InsertAfter(*tmp->pCurr, conv);
        }
        conv = tmp->pCurr->key;
    }
}
...
bool TPolynom::operator==(const TPolynom& tmp)const
{
    ...
    if ((a.pol->IsEnded()) && (b.pol->IsEnded()))
        return true;
    return false;
}
...
void TPolynom::InsertInSort(TMonom node)
{
    ...
    if (pol->pCurr->key == node.key)
    {
        pol->pCurr->pData = pol->pCurr->pData + node.pData;
        return;
    }
    else if (pol->pCurr->key > node.key)
    {
